...
mod tests {
    ...
    // pruneMessages - old messages can be added since size limit is not enforced
    #[test]
    fn test_prune_messages_allows_old_messages() {
        let (store, db, _temp_dir) = create_test_store();

        let base_time = messages_factory::farcaster_time();

        // Create and merge 3 messages
        let add1 = messages_factory::user_data::create_user_data_add(
            FID_FOR_TEST,
            UserDataType::Pfp,
            &"http://example.com/pfp1.jpg".to_string(),
        ...
        // Try to merge a message that's older than the earliest (should succeed since limit not enforced)
        let old_add = messages_factory::user_data::create_user_data_add(
            FID_FOR_TEST,
            UserDataType::Url,
            &"https://example.com".to_string(),
            Some(base_time + 1 - 3600), // 1 hour earlier than add1
            None,
        );

        merge_message_success(&store, &db, &old_add);

        // Verify the old message was stored
        let result =
            UserDataStore::get_user_data_add(&store, FID_FOR_TEST, UserDataType::Url as i32)
                .unwrap()
                .unwrap();
        assert_eq!(result, old_add);
    }
}
