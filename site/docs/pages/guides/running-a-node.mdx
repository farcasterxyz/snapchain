...
This guide will get you set up with a Snapchain node on an AWS EC2 instance. Use an instance with at least 64 GB of RAM and 1000 GB of high-performance SSD storage. You can run Snapchain on any server you like and costs may vary depending on provider.


## Launch a new instance
1. In AWS, go to EC2 > Instances > Launch Instances
2. Give it a name and select Ubuntu Server 22.04 LTS (HVM), SSD Volume Type and 64-bit (x86). Choose an instance type with at least 64 GB of RAM (for example, `r6i.2xlarge`).
...
5. In Configure storage, select at least 1000 GB of gp3 storage (or another NVMe-backed SSD volume with comparable performance).
