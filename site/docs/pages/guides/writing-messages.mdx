...
import { zeroAddress, bytesToHex } from 'viem';
...
const account = privateKeyToAccount(CUSTODY_PRIVATE_KEY as `0x${string}`);
const accountPrivateKey = CUSTODY_PRIVATE_KEY === '<REQUIRED>' ? generatePrivateKey() : CUSTODY_PRIVATE_KEY;
...
const hubClient = USE_SSL
  ? getSSLHubRpcClient(HUB_URL, { username: HUB_USERNAME, password: HUB_PASS })
