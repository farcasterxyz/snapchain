...
1. `submitMessage` requests can optionally include `dataBytes` for better performance. If not provided, Snapchain will automatically encode the `data` field. The `hub-nodejs` builders handle this optimization in all versions `>=0.13.0`. If you want to manually optimize this:
    ```typescript
