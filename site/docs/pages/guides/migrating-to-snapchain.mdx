...
### Notable differences on the client side 
- `submitMessage` requests must all contain `dataBytes` for Snapchain. The `hub-nodejs` builders handle this, but if you're not using those you will need to manually update this. 
...
### Notable differences for Snapchain node runners
