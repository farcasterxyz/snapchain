...
* A block is not produced — after 1 second, consensus changes leadership according to the rotation. We can tolerate the failure of up to 1/3 of the block leaders.  
...
* Shard validator majority can reissue a shard before block finality — if shard validators are malicious, they can reissue a shard for a block before it gets finalized. 
...
It's also possible a bug or non-deterministic behaviour causes a breaking change. E.g. a consensus breaking change is made without a corresponding protocol version bump. In this case, the nodes will proceed as normal until they encounter a block with the breaking change. At which point, the merkle roots will not match and the nodes that are not upgraded will halt. If this happens to validators, then block production will halt until a new version is released with a fix.
...
4. **Unidirectional sync is slow**. A node can be “ahead” of another node for some accounts state and “behind” it for another account. In order for these nodes to get into sync, both of them must pull data from the other node (bidirectional sync) before any state change happens. In practice, this is challenging to implement and we rely on unidirectional sync which means that only some state converges.
